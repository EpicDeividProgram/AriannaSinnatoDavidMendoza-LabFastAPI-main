...
def crear_caracteristica(db: Session, caracteristica: schemas.Caracteristica_a_Crear):
    respuesta_compra = compra_service.get_compra(db=db, id=caracteristica.encargo_id)
    if not respuesta_compra.ok:
        return Respuesta[schemas.Caracteristica](ok=False, mensaje='no existe el id del encargo')
...
def aprobar_caracteristica(db: Session, id_caracteristica: int):
    ...
    if caracteristica_found == None:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='Característica que desa aprobar no se a encontrado')
    caracteristica_found.estado_caracteristica_id = 2
    db.commit()

    return Respuesta[schemas.Cotizacion](ok=True, mensaje='Característica aprobada')
...
def rechazar_caracteristica(db: Session, id_caracteristica: int):
    ...
    if caracteristica_found == None:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='Característica que desea aprobar no se a encontrado')

    caracteristica_found.estado_caracteristica_id = 3
    db.commit()

    return Respuesta[schemas.Cotizacion](ok=True, mensaje='Característica rechazada')
...
def get_caracteristicas_encargo(db: Session, id_encargo: int): 
    ...
    return Respuesta[list[schemas.Caracteristica]](ok=True, mensaje='Características de el encargo encontradas', data=caracteristicas)
...
def modificar_caracteristica(db: Session, id: int, caracteristica: schemas.Caracteristica_a_Crear): 
