...
import anecdotaaaaaaaaaaa.service as anecdota_service
...
def get_productos(request: Request, db: Session = Depends(get_db), info=Depends(auth_handler.auth_wrapper)):
    ...
    if (lista_productos_respuesta.ok and
        ...
        raise mensage_para_Redirection_de_Exception(message=lista_productos_respuesta.mensaje, path_message='Volver a inicio', path_route='/')
...
def get_producto(request: Request, id : int, db: Session = Depends(get_db), info=Depends(auth_handler.auth_wrapper)): 
    ...
    if (not producto_respuesta.ok):
        raise mensage_para_Redirection_de_Exception(message=producto_respuesta.mensaje, path_message='Volver a inicio', path_route='/')

    artesano = usuario_service.buscar_usuario(db=db, cedula=producto_respuesta.data.usuario_cedula)
    if (not artesano.ok):
        raise mensage_para_Redirection_de_Exception(message=artesano.mensaje, path_message='Volver a inicio', path_route='/')
    ...
    if (not tipo_producto.ok):
        raise mensage_para_Redirection_de_Exception(message=tipo_producto.mensaje, path_message='Volver a inicio', path_route='/')

    categoria = categoria_service.get_categoria(db=db, categoria_id=producto_respuesta.data.categoria_id)
    if (not categoria):
        raise mensage_para_Redirection_de_Exception(message=categoria.mensaje, path_message='Volver a inicio', path_route='/')
    ...
    reseñas = reseña_service.listar_las_reseñas_de_productos_disponibles(db=db, id=producto_respuesta.data.id)
...
def create_producto(nombre: str = Form(...), 
    ...
    nuevo = schemas.ProductoCrear(
        ...
        profundidad_cm=profundidad_cm, imagen=imagen_data, 
